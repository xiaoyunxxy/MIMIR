...
def get_semisup_dataloaders(train_dataset, test_dataset, val_dataset=None, batch_size=256, 
                            batch_size_test=256, eff_batch_size=256, num_workers=4, 
                            unsup_fraction=0.5, num_replicas=None, rank=None):
    """
    ...
    train_batch_sampler = SemiSupervisedSampler(train_dataset.sup_indices, train_dataset.unsup_indices, batch_size, 
                                                unsup_fraction, num_batches=int(np.ceil(dataset_size/eff_batch_size)),
                                                dataset=train_dataset, num_replicas=num_replicas, rank=rank)
    ...
    if val_dataset:
        val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size_test, 
            persistent_workers=True if kwargs['num_workers'] > 0 else False, **kwargs)
...
class SemiSupervisedSampler(torch.utils.data.DistributedSampler):
    ...
    """
    def __init__(self, sup_inds, unsup_inds, batch_size, unsup_fraction=0.5, num_batches=None, dataset=None, num_replicas=None, rank=None):
