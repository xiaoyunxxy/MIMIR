...
def build_dataset(args, num_aug_splits=0):
    ...
    for i, item in enumerate(dataset_train.imgs):
        if 'train/n04266014/n04266014_10835.JPEG' in item[0]:
            corrupted_id = i
            break
    print('remove corrupted imagenet img.')
    dataset_train.imgs.pop(corrupted_id)
    dataset_train.targets.pop(corrupted_id)
    print(dataset_train)
    ...
    if args.no_aug:
        ...
        dataset_train.transform = tmp_transform
    else:
