...
def main(args, args_text):
    ...
    sampler_eval = OrderedDistributedSampler(sub_imagenet_set)
    loader_eval = torch.utils.data.DataLoader(
        dataset=sub_imagenet_set,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_metrics = train_one_epoch(
                epoch, model, loader_train, optimizer, train_loss_fn, args, device=device,
        ...
        eval_metrics = validate(model, loader_eval, validate_loss_fn, args, device=device, amp_autocast=amp_autocast, _logger=_logger)

        # model ema update
        if model_ema is not None and not args.model_ema_force_cpu:
            if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
                distribute_bn(model_ema, args.world_size, args.dist_bn == 'reduce')
            ema_eval_metrics = validate(model_ema.module, loader_eval, validate_loss_fn, args, device=device, amp_autocast=amp_autocast, log_suffix=' (EMA)', _logger=_logger)
...
def train_one_epoch(
        epoch, model, loader, optimizer, loss_fn, args, device=None,
...
def validate(model, loader, loss_fn, args, device=None, amp_autocast=None, log_suffix='', _logger=None):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
